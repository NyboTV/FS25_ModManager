...
async function syncMods(profileId: string, serverUrl: string): Promise<{ success: boolean, error?: string, message?: string }> {
  ...
  try {    // Lade Einstellungen und Profil
    ...
    if (profileData.autoSync && profileData.serverSyncUrl) {
      ...
      syncResult = await syncMods(profileId, profileData.serverSyncUrl);
        ...
        logger.warn(`Auto-Sync fehlgeschlagen: ${syncResult.error || 'Unbekannter Fehler'}`);
      }
    }
      // Starte das Spiel
    const { spawn } = require('child_process');
...
// Hier wird keine Funktion benötigt, da wir bereits eine syncMods-Funktion haben
// Die doppelte Implementierung wurde entfernt
